...
async function processBills() {
  ...
  for (let bill = await cursor.next(); bill != null; bill = await cursor.next()) {
      ...
      await bill.save();
      console.log("Bill saved: ", bill.BillID);
...
async function fetchAll() {
  try {
      ...
      // https://knesset.gov.il/Odata/ParliamentInfo.svc/KNS_Bill()?$filter=KnessetNum eq 25 and StatusID eq 108 or StatusID eq 141
...
async function fetchData(url: string, accumulatedData: unknown[] = []): Promise<unknown[]> {
  try {
...
export async function GET() {
  ...
  try {
    ...
    const storedBills = await Bill.find({});
    console.log(`bills in API: ${fetchedBills.length}`);
    console.log(`bills in database: ${storedBills.length}`);
    ...
    console.log(`Deleted ${deletedBillsCounter} bills`);
    
    // Insert new bills
    let insertedBillsCounter = 0;
    ...
    console.log(`Inserted ${insertedBillsCounter} new bills`);
